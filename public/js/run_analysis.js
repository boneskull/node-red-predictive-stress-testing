var apiUrl =
  location.protocol + '//' + location.host + location.pathname + 'red/';
...
function Process(
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      // update header
      var holdings_title =
        'Projected impact on your portfolio resulting from a ' +
        formShockMag +
        '% ' +
        formRiseFall +
        ' in the ' +
      ...
      var tf = '';
      tf +=
        "<tr tabindex='0' aria-label=Portfolio Table><td><strong>Portfolio Total</strong></td><td></td><td></td><td>" +
        SumCurrent +
        ' ' +
        BaseVal_Array[1] +
        "</td><td align='left'>" +
